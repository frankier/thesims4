...
class Patient:
    def __init__(self, env, pipeline, params):
        ...
        self.prep_time = randexp(params['mean_prep_time'])
        self.surgery_time = randexp(params['mean_surgery_time'])
        self.recovery_time = randexp(params['mean_recovery_time'])
...
def sample_experiment(num_samples, run_experiment, columns, sample_fn,
                      reseed=False):
    i = 0
    data = pandas.DataFrame(columns=columns, dtype=float)
    for i in range(num_samples):
        if reseed:
            random.seed(RANDOM_SEED + i)
        print('sample', i)
        pipeline, monitor = run_experiment()
        data = data.append(
            pandas.DataFrame(
                sample_fn(pipeline, monitor),
